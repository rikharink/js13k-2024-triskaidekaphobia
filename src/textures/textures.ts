...
export function generateTextureFromParagraph(
  ...
  const textCanvas2 = new OffscreenCanvas(1, 1);
  const tctx2 = textCanvas2.getContext('2d')!;

  return new Promise((resolve, reject) => {
    const svgCode = `
      <svg width="${size[0]}px" height="${size[1]}px" xmlns="http://www.w3.org/2000/svg">
          <foreignObject x="0" y="0" width="${size[0]}px" height="${size[1]}px">
              <style>
              p {
                  margin:0;
                  color: ${textOptions.fillStyle};
    ...
    textCanvas2.width = size[0];
    textCanvas2.height = size[1];
